...
class GoogleEventsList extends Component {
  ...
  grabEvents = (auth, date) => {
    ...
    const searchEnd = date.add(1, 'days').toISOString()
  ...
  render() {
    ...
    const eventItems = events.map((event) =>
      <Event id={event.id} summary={event.summary} location={event.location} start={event.start.dateTime} description={event.description}/>
    );
